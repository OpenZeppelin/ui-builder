...
- Adapters MUST publish an **EcosystemExport** (`ecosystemDefinition`) from `@openzeppelin/adapter-*`
  packages: metadata, `networks`, `capabilities`, and **`createRuntime(profile, networkConfig, options)`**
  returning an `EcosystemRuntime` whose fields are capability implementations.
- The builder MUST resolve runtimes through dynamic ecosystem registration via the
...
capability factories, `createRuntime` profiles, ecosystem registration, and strict
`EcosystemExport` / peer version compliance.
