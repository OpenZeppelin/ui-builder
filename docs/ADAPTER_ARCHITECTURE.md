...
This project uses a centralized service (`@openzeppelin/ui-utils/appConfigService`) for this purpose.
...
1.  **`UserRpcConfigService`**: A dedicated service in `@openzeppelin/ui-utils` that handles saving, retrieving, and managing user-defined RPC endpoints in `localStorage`.
...
2.  **`UserExplorerConfigService`**: A centralized service in `@openzeppelin/ui-utils` manages the storage and retrieval of these configurations from `localStorage`.
...
The `@openzeppelin/ui-storage` package provides persistent storage for contract UI configurations:
